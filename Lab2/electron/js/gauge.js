// https://dev.to/madsstoumann/how-to-create-gauges-in-css-3581
...
class AnalogGauge extends HTMLElement {
	...
	attributeChangedCallback(name, oldValue, newValue) {
		if (name !== 'value' || isNaN(newValue)) {
			return;
		}
		this.#stopGaugeLoop();
		this.#startGaugeLoop(-0.1, Number(newValue));
	}

	#startGaugeLoop (oldValue, newValue) {
		this.#intervalId = setInterval(() => {
			if (oldValue >= newValue) {
				clearInterval(this.#intervalId);
				return;
			}
			oldValue = (oldValue+10 < newValue) ? (oldValue + 10) : newValue;
			this.#units.value = parseFloat(oldValue || 0);
			this.#update();
		}, 25);
	}
