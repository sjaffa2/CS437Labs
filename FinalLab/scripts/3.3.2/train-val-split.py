...
for item in img_file_list:
    ...
    file_path = os.path.join(current_directory, 'custom_data/labels', basename + ".txt")
    if os.path.isfile(file_path):
        continue
    print(f"The file {file_path} does not exist or is not a file.")
    remove_list.append(item)
for item in remove_list:
    img_file_list.remove(item)

remove_list = []
for item in txt_file_list:
    basename = Path(item).stem
    file_path = os.path.join(current_directory, 'custom_data/images', basename + ".jpg")
    if os.path.isfile(file_path):
        continue
    print(f"The image {file_path} does not exist or is not a file.")
    remove_list.append(item)
for item in remove_list:
    txt_file_list.remove(item)
